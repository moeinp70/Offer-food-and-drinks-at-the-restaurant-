...
(defrule question-cuisine
    (declare (salience 85))
    ?state <- (state question)
    (not (asked cuisine))
    =>
    (printout t "What cuisine would you prefer ?" crlf)
    (printout t "(Seafood/Western/Japanese/Chinese/Indian)" crlf)
    (bind ?response (read))
    (switch ?response
        ...
        (default (printout t "Great, l'll search across all cuisines!" crlf)))
...
(defrule question-taste
    (declare (salience 65))
    ?state <- (state question)
    (not (asked taste))
    =>
    (printout t "What taste would you choose your  preference? ( sweet , spicy , sour) " crlf)
    (bind ?response (read))
    (switch ?response
        (case spicy then
        ...
        (default
            (printout t "Didn't catch that. l'll search across all taste." crlf)
            ))
...
(defrule question-value
    ...
    (printout t " what's your tolerance of your chosen taste? (scale of 0-2)" crlf)
    (bind ?response (read))
    (switch ?response
        (case 0 then
            (assert (preference (property taste-val) (numberValue 0))))
        (case 1 then
            (assert (preference (property taste-val)(numberValue 1))))
        (case 2 then
            (assert (preference (property taste-val)(numberValue 2))))
        (default
            (assert (preference (property taste-val) (numberValue 1)))))
...
(defrule process-value
    ...
    (declare (salience 58))
    (preference (property taste-val)(numberValue ?preferred))
...
(defrule question-other
    (declare (salience 35))
    ?state <- (state question)
    (not (asked other))
    =>
    (printout t "Do you want to specify any other preferences? (yes/no)" crlf)
    (bind ?response (read))
    (if (eq ?response yes)
    then
        (printout t "Let me ask you a few more questions about your other preferences." crlf)
...
(defrule start-drink-module
    ...
    (switch ?response
        ...
        (default (printout t "Great, so I'll search across all drinks!" crlf)))
...
(defrule terminate0
    ...
    (state terminated0)
    =>
    (printout t "Would you like to try prefer dish? (yes/no)" crlf)
    (bind ?response (read))
    (if (eq ?response yes)
    then
        (reset)
